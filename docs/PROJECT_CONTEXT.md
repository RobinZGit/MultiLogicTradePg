...
**Последнее обновление:** 2026-07-12 (SMAT3 свёртка / SMAT3COMP композиция, справка @CODE)
...
- **`sma(pp)`**, **`ema(pp)`**, **`ww()`**, `@CODE`, `*`, `#`, ядра `(1;-2;1)` — единый парсер `poly_*` в `02`;
...
21. **Фоновый пересчёт после drag:** POST assign → список сразу; `POST /sync` с `async:true` + `indicator_id` → PostgreSQL в фоне; спиннер «Пересчёт PACC…» у кнопки загрузки цен.
...
21. «SMAT3 — свёртка sma(pp)*ww()*ww(); старый sma(sma(sma)) → SMAT3COMP; справка @ vs sma; каталог всех индикаторов в help».
