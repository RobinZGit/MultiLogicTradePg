...
- **`*`** — свёртка **уже вычисленных** рядов (без подмены ядром); **`sma(sma(…))`** — композиция; SMAT3 ≠ SMAT3COMP;
...
| 2026-07-12 | `*` = буквальная свёртка; SMAT3 (sma*3) ≠ SMAT3COMP (композиция) |
...
23. «* = буквальная свёртка вычисленных рядов; SMAT3 ≠ SMAT3COMP; обновить help/контекст; БД с нуля; в репо».
